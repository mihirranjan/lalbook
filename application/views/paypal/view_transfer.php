      ...
      <div class="clsTabs clsInnerCommon clsInfoBox">
							...
							if($msg = $this->session->flashdata('flash_message'))
							  ...
							  <form name="drop_list" action="<?php echo site_url('transfer'); ?>"  method="post">
									  ...
									  foreach($projectList_tranferamount as $res)
										{ 
										  ...
										  if($logged_userrole == '1')
											{
											  if($res->creator_id == $loggedInUser->id and $res->employee_id != '0')
												{ ?>
								 ...
								 <p><span><?php if($loggedInUser->role_id == '1') echo $this->lang->line('Select Employee'); else echo $this->lang->line('Select Owner');?>:</span>
								<select name="users_load" id="users_load" class="clsListBox">
								 												
									 <?php 
									 if($logged_userrole == '2')
									   { ?>
										 <option value="0" selected="selected"><?php echo '<b>-- '.$this->lang->line('Select Owner').' --</b>'; ?> </option> <?php 
									   } 
									 if($logged_userrole == '1')	
									   { ?>
									     <option value="0" selected="selected"> <?php echo '<b>-- '.$this->lang->line('Select Employee').' --</b>'; ?> </option> <?php 
									   } ?>
								</select>
							  </p>
                          ...
                          <!--END OF SEND MONEY-->
							<p><span>&nbsp;</span><input  class="clsLogin_but" name="transferMoney"  value="<?php echo $this->lang->line('Transfer');?>" type="submit" onclick="javascript:return formValidation();"/> </p>
							</form>
							</div>
							<br />
...
function load_user()
{	//alert("test");//
	var a=$('#type_id').val();//alert(a);
/*	var url = '<?php echo site_url('users/load_users');?>';
	new Ajax.Updater('users_load', url,   {  method     : 'post',
	  parameters : { type_id : a },
	  onLoading  : function ()
	  {
		$('#users_load').html('<img alt="loading..." src="<?php echo base_url().'images/loading.gif' ?>" />');
	  }
});*/ //Ajax Object Creation End
new Ajax.Request('<?php echo base_url().'index.php/users/load_users';?>',
	  {
		method:'post',
		 parameters : { type_id : a },
		onSuccess: function(transport){
		  ...
		  document.getElementById('users_load').innerHTML = response;
		},
...
function formValidation()
{
	...
	//This is used to check the job name field
	if($('type_id').value == '0')
	  {
