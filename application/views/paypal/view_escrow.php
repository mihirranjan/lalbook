...
<div id="main">
      ...
      <div class="clsTabs clsInnerCommon clsInfoBox">
							...
							<div class="clsEditProfile clsSitelinks"> 
                              <h3><span class="clsEscrowfund"><?php echo $this->lang->line('Escrow Funds');?></span></h3>
							...
							<p><span><?php echo $this->lang->line('User name :');?></span><a class="glow" href="<?php if($loggedInUser->role_id == '1') $res = 'owner'; else $res = 'employee'; echo site_url($res.'/viewprofile/'.$loggedInUser->id); ?>"> <?php echo $loggedInUser->user_name?></a>
...
<p><span><?php echo $this->lang->line('Account Balance:');?></span> <?php echo $currency;?> <?php if(isset($userAvailableBalance)) echo $userAvailableBalance.'.00'; ?></p>
							...
							if($msg = $this->session->flashdata('flash_message'))
							  ...
							  <form name="drop_list" action="<?php echo site_url('escrow'); ?>"  method="post">
								 <p><span> <?php echo $this->lang->line('Select Job'); ?> :</span>
								  <select id="type_id" name="type_id" onchange="javascript:load_user(this.value);">
									  <option value="0"><?php echo '-- '.$this->lang->line('Select Job').' --'; ?></option>
									  <?php 
									  foreach($projectList->result() as $res)
										{ 
										  //Make transfer only for owner to employee
										  if($logged_userrole == '1')
											{
											  if($res->creator_id == $loggedInUser->id and $res->job_id != '0')
												{ ?>
												  <option value="<?php echo $res->id; ?>" > <?php echo $res->id.' -- '.$res->job_name; ?></option> 
												  <?php 	
												}	
											}
										  //Make transfer only for employee to owner
										  if($logged_userrole == '2')
											{
											  if($res->job_id == $loggedInUser->id)
												{ ?>
												 <option value="<?php echo $res->id; ?>" > <?php echo $res->id.' -- '.$res->job_name; ?></option> <?php 	
												}	
											}	
									  }	//foreah end here  ?>
								 </select></p>
								 <div id="projectName" name="projectName" style="display:none; color:red;">
									<p><span>&nbsp;</span><span class="help" style="margin-left: 0px !important;"><?php echo $this->lang->line('Select Job'); ?></span></p>
								</div>
								 <p><span><?php echo $this->lang->line('Employee Name');?>:</span>
								 <b id="prog_id"> <select id="users_load" name="users_load"><option value="0"><?php echo $this->lang->line('Employee Name');?></option>
								 </select></b></p>
									  
							
 							
							
							<p><span><?php echo $this->lang->line('Job Amount'); ?></span><div id="project_amount"><?php echo $this->lang->line('N/A'); ?></div></p>
							...
							<p><span><?php echo $this->lang->line('Transfer Amount').' '.$currency; ?></span> <input name="total" id="total" size="10" value="<?php echo set_value('total'); ?>"  type="text"/></p>
                          ...
                          <!--END OF SEND MONEY-->
							...
							<p style="overflow:hidden;"><span style="color:#727272;font-style:italic;font-size:11px;">(<?php echo $this->lang->line('select');?>)</span></p>
							<p>
							  <!--<input type="submit" value="Deposit" name="depositMoney" class="clsLogin"/>-->
							</p>                       
							
							<p>  <input  class="clsLogin_but" name="transferMoney"  value="<?php echo $this->lang->line('Transfer');?>" type="submit" onclick="javascript:return deposit_confirm(document.drop_list.type_id.value, document.drop_list.total.value);"/> </p>
							</form>
							<br />
							</div>
							...
							<table cellspacing="1" cellpadding="2" width="96%">
                                <tbody><tr>
                                  ...
                                  <td width="23%" class="dt"><?php echo $this->lang->line('Amount');?></td>
								  <td width="20%" class="dt"><?php echo $this->lang->line('Date');?></td>
								   <td width="10%" class="dt"><?php echo $this->lang->line('Job');?></td>
								  ...
								  if($i%2 == 0)
									...
									$class1 = "dt2";
									  ...
									  <td class="<?php echo $class1; ?>"><?php foreach($usersList->result() as $user) { if($user->id == $res->creator_id) { ?>
									   <a href="<?php if($user->role_id == '1') echo site_url('owner/viewProfile/'.$user->id); if($user->role_id=='2') echo site_url('employee/viewProfile/'.$user->id);?>"> <?php  echo $user->user_name; break; } }  ?></a></td>
									  ...
									  <td class="<?php echo $class1; ?>"><?php echo get_datetime($res->transaction_time); ?></td>
									   <td class="<?php echo $class1; ?>"><?php $job_name= get_project_name($res->job_id); if(isset($job_name)) echo $job_name; ?></td>
...
function load_user(value)
{
   ...
   if(utype>0)
  {
	  ...
	  new Ajax.Request('<?php echo base_url().'index.php/escrow/escrowJob';?>'+'/'+utype,
	  {
		method:'post',
		onSuccess: function(transport){
...
// Function for check whether the transaction will be exceed the job amount
function is_exceed(projectId, amt)
{
	var opt=true;
	new Ajax.Request('<?php echo base_url().'index.php/escrow/escrowJob';?>'+'/'+projectId,
	{
		asynchronous:false,
		method:'post',
		onSuccess: function(transport){
