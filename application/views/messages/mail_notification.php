...
<div class="Container">
   ...
   <?php  foreach($notification->result() as $searchresult)
						    {
							$jobidd=$searchresult->creator_id;
							}
							?>
    <?php 
	$uid=$loggedInUser->id;
	$qs = "update buy_requirement set notification_status='0' WHERE creator_id = $uid";
...
<th> Job Type	</th>
 ...
 <?php   if(isset($notification) and $notification->num_rows()>0)
						 {
						 $i=0;
						 $a=0;
						//echo $buyrequirement->num_rows();
						  foreach($notification->result() as $searchresult)
						    {
							if($i%2==0){
						$clsodd="";
						}else{
						...
						$a++;
							...
							//echo $jobidd;
							$q = "SELECT COUNT(id) AS count FROM bids WHERE job_id = $jobidd";
							//echo "SELECT COUNT(job_id) AS count FROM bids WHERE job_id = $jobidd";
 ...
 <tr class="" >
 <td><?php echo $a;?></td>
 <td><a href="<?php echo site_url('job/view/'.$searchresult->buy_id);?>"><?php echo $searchresult->looking_for;?></a></td>
 <td> <?php echo $searchresult->category;?>	</td>
 <td> <?php echo $searchresult->budget;?></td>
 <td> <?php echo $searchresult->status;?></td>
 <td> <?php $status=$searchresult->status;
 if($status=='closed'){ echo "Job accepted";} else { echo "Job awarded";}?></td>
 </tr>
 <?php $i++;}
 }
 else{
