...
<p><b><?php echo $this->lang->line('Maventricks'); ?></b>
...
<?php  $res = count($job_name); $netamount = 0;
for($i=1;$i<=$res;$i++)
  { $k=0;
  
    foreach($job_name as $name)
	  { 
	   $rec = explode('--',$name);
	   ?>
		 <tr><td><?php echo $k=$k+1; ?></td><td><?php echo $rec[0].' -- '.$rec[1]; ?></td><td><?php foreach($bidsjobs->result() as $row) {  if( $rec[0] == $row->job_id ) { echo $row->bid_amount; $netamount = $netamount + $row->bid_amount; } }//echo $this->lang->line('amount'); ?></td></tr> <?php 
      }break;
  }
 ?><br />
 <tr><td></td></tr>
