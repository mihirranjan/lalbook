...
<?php
		...
		$job = $jobs->row();
		if(is_object($bid))
		   $action = site_url('job/editBid');
		else
		   $action = site_url('job/createBid/'.$job_id);
?>
<!--MAIN-->
 <?php
		//Show Flash Message
...
<div id="main">
      ...
      <div class="clsInnerpageCommon">
                              ...
                              <h2><?php echo $this->lang->line('Bid on Job'); ?>:<?php echo $job->job_name; ?></h2>
							  <div class="clsForm">
							  <form method="post" action="<?php echo $action; ?>" name="myForm">
								...
								<div id="selPMB" class="clsMarginTop">
								  ...
								  <p class="clsSitelinks"><?php echo $this->lang->line('You are currently logged in as');?> <a class="glow" href="<?php echo site_url('employee/viewProfile/'.$this->loggedInUser->id);?>">
								  ...
								  <p><label><?php echo $this->lang->line('Your Bid'); ?>:</label><?php echo $currency;?> <input name="bidAmt" type="text" value="<?php if(isset($bid->bid_amount)) echo $bid->bid_amount; else echo set_value('bidAmt');?>" size="8" />
								  <?php echo form_error('bidAmt'); ?>
								  </p>
								  
								  <p><input type="checkbox" name="notify" value="1"/> <?php echo $this->lang->line('Notify me by e-mail if someone bids lower than me on this job.');?></p>
						  ...
						  <p><input type="checkbox" name="escrow"  value="1" <?php if(isset($bid->escrow_flag)){if($bid->escrow_flag=='1') echo 'checked="checked"';}?>/> <?php echo $this->lang->line('Required Escrow');?></p>
							    ...
							    <?php echo $this->lang->line('Characters Left') ?>
									...
									<p><label>&nbsp;</label>
									<span><?php echo $this->lang->line('Tip');?></span> </p>
								
								    <p><label>&nbsp;</label>
									<input class="clsLogin_but" type="submit" value="<?php echo $this->lang->line('Submit'); ?>" name="postBid"/>
