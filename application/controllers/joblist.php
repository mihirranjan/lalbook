...
class Joblist extends CI_Controller {
	...
	function __construct()
	{
		...
		{
			$condition = array('draftjobs.creator_id' => $this->loggedInUser->id);
	...
	{	
		...
		if(!isBuyer())
		{
        	$this->session->set_flashdata('flash_message', $this->common_model->flash_message('error',$this->lang->line('You must be logged in as a owner to post projects')));
		   ...
		   redirect('information/index/success');	
			   ...
			   else
			   {
				...
				$joblistvalidity  = $paymentSettings['JOBLIST_VALIDITY_LIMIT'];
				  ...
				  {
					$insertJobData              		  	= array();	
				    $insertJobData['job_name']  	  	= $this->input->post('projectName');
					...
					if($this->input->post('categories'))
					{
						...
						$project_categoriesNameArray 	           = $this->skills_model->convertCategoryIdsToName($categories);
						$project_categoriesNameString              = implode(',',$project_categoriesNameArray);
						$insertJobData['job_categories']          = $project_categoriesNameString;
					}
					
					if($insertJobData)            
					  {
						$this->skills_model->draftJob($insertJobData);
						$this->session->set_flashdata('flash_message', $this->common_model->flash_message('success',$this->lang->line('Your Job has been saved as Draft')));
		   redirect('information/index/success');	
					  }
					  else
			   ...
			   { 
				...
				$updatejobDraft['job_name']  	  	= $this->input->post('projectName');
				...
				if($this->input->post('categories'))
				{
					...
					$project_categoriesNameArray 	           = $this->skills_model->convertCategoryIdsToName($categories);
					$project_categoriesNameString              = implode(',',$project_categoriesNameArray);
					$updatejobDraft['Job_categories']         = $project_categoriesNameString;
				}
				if($updatejobDraft)            
				  {
					$condition = array('draftjobs.id'=>$draft->id);
		...
		$conditions   = array('job.id'=>$project_id,'jobs.creator_id'=>$this->loggedInUser->id);
				...
				{	 $insertJobData['attachment_url']=$this->data['file']['file_name'];  $insertJobData['attachment_name']=$this->data['file']['orig_name']; }	
				  ...
				  if($this->input->post('preview_project'))
		          {
					  ...
					  //joblist data
					   ...
					   $result                                  = '1';
					   $outputjobData['job_status']  	= 'Pending';
					   $outputjobData['job_name']  	  	= $this->input->post('projectName');
					   $paymentSettings = $this->settings_model->getSiteSettings(); 
				$joblistvalidity  = $paymentSettings['JOBLIST_VALIDITY_LIMIT'];
					  ...
					  if($this->input->post('createProject'))
						...
						if($this->input->post('is_hide_bids',TRUE) or $this->input->post('is_urgent',TRUE) or $this->input->post('is_feature',TRUE) or  $this->input->post('is_private',TRUE)) 
						{
							...
							else
							{
								...
								$insertTransaction['type'] 		 = $this->lang->line('Job Fee');
...
/**
	 * view project by owner or employee
	 *
	 * @access	private
	 * @param	nil
	 * @return	void
	...
	{	
		...
		if(isset($project_id) and isset($this->loggedInUser->id))
		  {
			$updateKey                         = array('job_invitation.job_id'=>$project_id,'job_invitation.receiver_id'=>$this->loggedInUser->id);
		...
		if(isset($this->loggedInUser->id) and $projects->creator_id==$this->loggedInUser->id)
			...
			$conditions = array('bids.job_id'=>$project_id,'projects.is_hide_bids' => 0);
	...
	/**
	 * view all the jobs by owner or employee 
	 *
	 * @access	private
	 * @param	nil
	 * @return	void
	...
	function viewAlljoblists()
	{
	  ...
	  $type = $this->uri->segment(3,'0');
	   ...
	   //Check list only Joblist job
	   $project_falg=1;
	   $projectstatus=0;
	    if($type == 'flag')
		{	
		$order=array('jobs.id','desc');
		$joblist_conditions =array('jobs.flag'=>'1','jobs.job_status'=>'0');
		$joblist = $this->skills_model->getJobs($joblist_conditions,NULL,NULL,$max,$order);
		$jobs = $this->skills_model->getJobs($joblist_conditions);
		...
		$order=array('jobs.id','desc');
		$joblist_conditions =array('jobs.flag'=>'1','jobs.job_status'=>'0');
		$joblist = $this->skills_model->getJobs($joblist_conditions,NULL,NULL,$max);
		$jobs = $this->skills_model->getJobs($joblist_conditions);
		 $this->outputData['pName'] = 'Job Listing';
		}
		...
		if($this->loggedInUser->role_id)
		  {
		  	if($this->loggedInUser->role_id == '2')
			  {
			  	$this->session->set_flashdata('flash_message', $this->common_model->flash_message('error',$this->lang->line('You must be logged in as a buyer to post jobs')));
			    redirect('information');
			  }
		  }
		$project_id    =    $this->uri->segment(3,0);
...
/**
	 * discard draft Job and job by owner
	 *
	 * @access	private
	 * @param	nil
	 * @return	void
	...
	function manageJoblist()
	{
	   ...
	   $update    =    $this->input->post('update'); 
		...
		if($this->form_validation->run())
		{
			...
			//Create job before it for update the jobs datas for manage option	   
