...
/*
  ...
  ***      BIDONN 1.0                                                      ***
  ***      File:  escrow.php                                               ***
  ***      Built: Mon June 23 09:16:10 2012                                ***
...
class Escrow extends CI_Controller {
	...
	{	
		...
		if($this->input->post('transferMoney'))
		{
			...
			if($this->form_validation->run() and $this->input->post('type_id') != '0')
			{
				  ...
				  $bal_amount = $avail_balance  -  ( $paymentSettings['PAYMENT_SETTINGS'] +  $this->input->post('total') );
				    ...
				    if( $bal_amount < 0)
				    {
						$this->session->set_flashdata('flash_message', $this->common_model->flash_message('error',$this->lang->line('You donot have enough balance in your account to escrow the money')));
			            redirect('escrow');
					}
				  else
				    {
					  ...
					  $method  =  'Paypal';
					  $condition = array('jobs.id'=>$this->input->post('type_id'));
					  $jobData   = $this->skills_model->getMembersJob($condition);
		              $usersdata = $jobData->row();
					  
					  $this->outputData['amount']  	  		= $this->input->post('total');
					  //Register Transaction
					  $insertData = array(); 
					  $insertData['creator_id']   			= $this->loggedInUser->id;
					  $insertData['reciever_id'] 			= $usersdata->employee_id;
					  $insertData['employee_id'] 			= $this->input->post('prog_id');
					  ...
					  $res = $this->account_model->addTransaction($insertData);
					    ...
					    if(getSuspendStatus($usersdata->employee_id))
					  {
					  ...
					  if($results->num_rows()>0)
						{
						 ...
						 //Send email to the user after made payments
						  ...
						  $splVars = array("!site_name" => $this->config->item('site_title'),"!username" => $this->loggedInUser->user_name,"!siteurl" => site_url(),"!amount"=>$this->input->post('total'),"!type"=>'Escrow',"!others1"=>'Job Name   :'.$jobs->job_name, "!contact_url" => site_url('contact'));
		...
		$this->load->view('paypal/view_escrow',$this->outputData);
	} //Function index End
		...
		if($this->uri->segment(3,0))
		  {
			...
			$this->account_model->addescorwRelease($insertData);
			...
			$transaction          = $this->account_model->getallTransactions($condition);
			$transaction          = $transaction->row();
		    $employee_id          = $transaction->employee_id; 
			$amount               = $transaction->amount;     
			$condition_balance 	  = array('user_balance.user_id'=>$employee_id);
			$employee_balance     = $this->account_model->getBalance($condition_balance);
			

			if($employee_balance->num_rows()>0)
			{
				$progBalance = $employee_balance->row();	
				$avail_balance_prog =  $progBalance->amount;
			
				$updateKey 			  = array('user_balance.user_id'=>$employee_id);	
				$updateData 		  = array();
								
							
			 	$updateData['amount'] = $avail_balance_prog   +   $amount;
			 	$results1 			  = $this->account_model->updateBalance($updateKey,$updateData);
			...
			$condition  		  =  array('jobs.id'=>$transaction->job_id); 
			$jobs         	      =  $this->skills_model->getMembersJob($condition);
			$jobs            	  =  $jobs->row();
			...
			$splVars = array("!site_name" => $this->config->item('site_title'),"!username" => $this->loggedInUser->user_name,"!siteurl" => site_url(),"!amount"=>$transaction->amount,"!type"=>'Escrow Release',"!others"=>'Receiver Name   :'.$registerusers->user_name,"!others1"=>'Job Name   :'.$jobs->job_name, "!contact_url" => site_url('contact'));
 ...
 /*
	...
	*/
		...
		if($this->uri->segment(3))
		  {
			...
			$splVars = array("!site_name" => $this->config->item('site_title'),"!username" => $this->loggedInUser->user_name,"!siteurl" => site_url(),"!amount"=>$transaction->amount,"!type"=>'Escrow Cancel',"!others"=>'Receiver Name   :'.$registerusers->user_name,"!others1"=>'Job Name   :'.$jobs->job_name, "!contact_url" => site_url('contact'));
			$mailSubject = strtr($rowUserMailConent->mail_subject, $splVars);
			$mailContent = strtr($rowUserMailConent->mail_body, $splVars);		
			$toEmail     = $this->loggedInUser->email;
			$fromEmail   = $this->config->item('site_admin_mail');
	...
	{ 
		...
		// Terminate the process if the job id is 0
		...
		// Count the escrow transactions of the job
		
		$condition=array('transactions.type'=>'Escrow Transfer', 'transactions.job_id'=>$job_id);
		...
		$escrow_due_rec=$this->skills_model->getJobs($condition, 'jobs.escrow_due, jobs.employee_id');
		$escrow_due_row=$escrow_due_rec->row();
		$escrow_due=$escrow_due_row->escrow_due;
		
		$escrow_due=($escrow_due==0)?0:($escrow_due-1);	
		
		// Get the job bid amount
		$condition=array('bids.job_id'=>$job_id, 'bids.user_id'=>$escrow_due_row->employee_id);
		$condition;
		$bids_res=$this->skills_model->getBids($condition);
		$bids=$bids_res->row();
		
	    $job_bid_amount=$bids->bid_amount;	
		
		// Get the paid amount of the job
		$job_paid_amount=0;
		$qry="SELECT SUM(amount) AS job_paid_amount FROM transactions WHERE job_id=$job_id AND (type='Escrow Transfer' OR type='Transfer') AND status='Completed' GROUP BY job_id";
		$transactions_rec=$this->db->query($qry);
		if($transactions_rec->num_rows()>0)
		{
			$transactions=$transactions_rec->row();
			$job_paid_amount=$transactions->job_paid_amount;
		}
 		
		$job_rem_amount=($job_bid_amount-$job_paid_amount);
		$job_rem_amount=($job_rem_amount<0)?0:$job_rem_amount;
		$job_rem_amount=sprintf('%.2f', $job_rem_amount);
		
		$reply=$job_bid_amount.'#'.$job_rem_amount.'#'.$no_of.'#'.$escrow_due;
...
//------------------------------------------------------------------------//
	...
	function load_users()
	{
		if($this->input->post('type_id'))
		{
			...
			$job_id = $this->input->post('type_id');		
...
/* Location: ./application/controllers/escrow.php */
?>