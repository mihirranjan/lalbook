...
class Seller extends CI_Controller {
	 ...
	 */ 
	 ...
	 //load validation libraray
		...
		$condtition = array('users.user_status'=>1);
		...
		foreach($buye as $buyer)
			...
			/*print_r($this->outputData['bidjob']); exit;*/
			 //echo $avlbcredit;exit;
		...
		//$this->outputData['bidsno'] =  $this->requirement_model->getSellerview($condtition);
		//print_r($this->outputData['categorries']);exit;
		//print_r($this->outputData['buyrequirement']);exit;
			 ...
			 else if($keywords=='' &&  $budgets!='' && $countrys!='' && $categorys!='' && $bsntype!='')
		{
		 $conditions =" b.creator_id = u.id  AND b.requirements = '$bsntype' AND u.country_symbol = '$countrys'  AND b.category = '$categorys' ";
			...
			//print_r($result);exit;
			 //return $this->db->get();
			//print_r($query);
		//	 $this->outputData['myOpportunities']  =  $this->skills_model->getSearchJobs(NULL,NULL,$like1,NULL,NULL);
	/*	
		if($like1){
	echo	$query = "SELECT *  FROM buy_requirement as b where ".$conditions." or  ".$like1."";
	    $jobresult = $this->db->query($query);
		}else{
	echo	$query = "SELECT *  FROM buy_requirement as b,users as u where ".$conditions."";
		}
	exit;		 */
	//echo $query;
			//print_r($this->outputData['myOpportunities']);exit;
		}
