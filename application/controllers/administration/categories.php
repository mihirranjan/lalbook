...
class Categories extends CI_Controller {
	...
	function __construct()
	{
	   ...
	   //Check For Admin Logged in
		...
		// loading the lang files
			$this->lang->load('admin/category', $this->config->item('language_code'));
	...
	{	
		...
		$this->load->view('admin/users/view_addBans',$this->outputData);
	...
	{	
		...
		$this->outputData['pagination']   = $this->pagination->create_links2(false,'editBans');
		 ...
		 $this->load->view('admin/users/view_editBans',$this->outputData);
	}//End of addBans Function
	...
	{	
		...
		if($this->input->post('addCategory')){
			//Set rules
			$this->form_validation->set_rules('catetype','Category_type','required');
			$this->form_validation->set_rules('category','Category','required|trim|xss_clean');
			
			if($this->form_validation->run())
			{	
			      $insertData                   = array();	
			       $insertData['cate_type']     	= $this->input->post('catetype');
				  $insertData['category']    	= $this->input->post('category');
				 
				
				  
				  //Insert User
				  $this->category_model->createCategory($insertData);
				
				  
				  
				  //Notification message
				  $this->session->set_flashdata('flash_message', $this->common_model->admin_flash_message('success',$this->lang->line('added_success')));
				  redirect_admin('categories/viewCategories');
			}
		}
		$this->load->view('admin/categories/view_addCategory',$this->outputData);
	}//End of addBans Function
	...
	//$condition = array('users.user_name'=>!'');
		...
		$order[0]            ='id';
		$order[1]            ='asc';
		$userbids=$this->user_model->getUserbids();
		$this->outputData['usrrequirement']=$userbids;
		//Get Groups
	    $products=	$this->category_model->getCategories();
		$this->outputData['userDetails'] = $products;
	//Pagination
		$this->load->library('pagination');
		$config['base_url'] 	 = admin_url('categories/viewProducts');
		$config['total_rows'] 	 = $userDetail->num_rows();		
		$config['per_page']     = $page_rows; 
		$config['cur_page']     = $start;
		$this->pagination->initialize($config);		
		$this->outputData['pagination']   = $this->pagination->create_links2(false,'viewProducts');
	 $this->load->view('admin/categories/view_products',$this->outputData);
	}//End of addBans Function
	...
	//$condition = array('users.user_name'=>!'');
		...
		$this->outputData['usrrequirement']=$userbids;
		//Get Groups
	    $products=	$this->category_model->getServices();
		$this->outputData['serviecedetails'] = $products;
	//Pagination
		$this->load->library('pagination');
		$config['base_url'] 	 = admin_url('categories/viewServices');
		$config['total_rows'] 	 = $userDetail->num_rows();		
		$config['per_page']     = $page_rows; 
		$config['cur_page']     = $start;
		$this->pagination->initialize($config);		
		$this->outputData['pagination']   = $this->pagination->create_links2(false,'viewServices');
	 $this->load->view('admin/categories/view_services',$this->outputData);
	}//End of addBans Function
	// --------------------------------------------------------------------
	...
	$this->load->model('category_model');
		...
		if($this->input->post('editCategory')){
			...
			{	
				  ...
				  $updateData['cate_type']     	= $this->input->post('catetype');
				  $updateData['category']    	= $this->input->post('category');
				  
				  
				  $updateKey = array('category.id' => $this->input->post('categoryid',true));
				  
				 		   
				  //pr($updateData);exit;
				  //Edit user
				  $this->category_model->updateCategory($updateKey,$updateData);
  ...
  //Notification message
		...
		//echo $userid;
		$condition = array('category.id' => $userid);
		$this->outputData['userDetails'] = $this->category_model->getCategories($condition);
		$this->load->view('admin/categories/view_editCategory',$this->outputData);
	}//End of addBans Function
	...
	{	
	$this->load->model('category_model');
		 $cateid = $this->uri->segment(4,'0');
		 $getUsers	=	$this->user_model->getUsers();
		 
		if($cateid==0)
		{
			$getUserss	=	$this->user_model->getUsers();
			 $userlist  =   $this->input->post('catelist');
			 if(!empty($userlist))
			  {
				foreach($userlist as $res)
				 {
				  $condition = array('category.id' => $res);
		          $getUser	=	$this->category_model->getCategories($condition);
							$condition = array('category.id' =>$res);
							$this->category_model->deleteCategory(NULL,$condition);
		 ...
		 {
		    $condition = array('category.id' => $cateid);
		    $getUser	=	$this->category_model->getCategories($condition);
	...
	*/
	...
	{	
		...
		$this->load->view('admin/users/view_admin',$this->outputData);
	}//End of addBans Function
	...
	if($adminid==0)
	{
		...
		if($this->input->post('manageAdmin'))
		  {
			...
			for($i=0;$i<$count;$i++)
			{
				...
				$updateKey['password']    	= hash('sha384',$password[$i]); 
	  ...
	  else
	  {
			 ...
			 $this->load->view('admin/users/view_editAdmin',$this->outputData);
	  }	
	}//End of Function
		...
		if($this->input->post('searchAdmin'))
		  {
			...
			{
			   $this->load->view('admin/users/view_searchAdmin');
			}   
		  }
		else
		  {
	...
	{	
      ...
      $this->load->view('admin/users/view_addAdmin',$this->outputData);
	}//End of addBans Function
