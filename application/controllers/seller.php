...
class Seller extends CI_Controller {
	 ...
	 */ 
	 function index()
	 {
		...
		$total =  $this->requirement_model->count_posts();
		$start_inbox =  $this->uri->segment(3,'0');
		$limit[0]			 = 20;
		$limit[1]			 = $start_inbox;
		if($start_inbox > 0)
			$start_inbox = ($start_inbox-1) * 20;
		  
		$this->load->library('pagination');
		$condt=array('buy_requirement.status'=>'open');
		$this->outputData['buyrequirement'] =  $this->requirement_model->getRequirement($condt);
		$userrequire=$this->outputData['buyrequirement'] ;
		$buye=$userrequire->result();
		//echo $userrequire->num_rows();exit;
		foreach($buye as $buyer){
			$avlbcredit=$buyer->buy_id;

			$cond=array('bids.job_id'=>$avlbcredit);
			$this->outputData['bidjob'] =  $this->requirement_model->getBuyerview($cond);
		 }
		
		
		//print_r($this->outputData['buyrequirement']);exit;
		$this->outputData['countries']	=	$this->common_model->getCountries();
		$this->outputData['categorries']	=	$this->common_model->getCategories();
		if($this->loggedInUser){
			...
			$reqtype=$this->outputData['userrecords'] ;
			$btype=$reqtype->result();
			
			foreach($btype as $bstype){
				$prdtype=$bstype->industry_type;
				// echo $prdtype;
				$condtn=array('buy_requirement.category'=>$prdtype,'buy_requirement.status'=>'open');
				// echo $condtn;
				$this->outputData['buyrequirement'] =  $this->requirement_model->getRequirements($condtn);
				$reqpage=$this->outputData['buyrequirement'] ;
				$totalrows= $reqpage->num_rows();
				$config['base_url'] 	 = site_url('seller/index');
				$config['total_rows'] 	 = $totalrows;		
				//echo $totalrows;exit;
				$config['per_page']     = 20; 
				$config['cur_page']     = $start_inbox;
				$this->pagination->initialize($config);		
				$this->outputData['pagination1']   = $this->pagination->create_links2(false,'seller/index');
			}
			 
		}
		
		
		$query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category as category,b.description,b.budget,b.requirement_image,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
		u.city,u.state,u.user_name  FROM buy_requirement as b,users as u where u.id=b.creator_id order by buy_id DESC";
		...
		if($this->input->post('search'))
		{
			...
			$bsntype=$this->input->post('bstype');
			$order=array('buy_requirement.id','desc');
			  
			  $like1 = '';
			  $conditions='';
			  
			if($keywords!='' && $budgets=='' && $countrys=='' && $categorys=='' && $bsntype==''){
				// $like1 = array('buy_requirement.looking_for'=>$keywords,'buy_requirement.category'=>$keywords,'buy_requirement.description'=>$keywords,'buy_requirement.budget'=>$keywords);
				 
				 $like1 = "b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state,u.user_name  FROM buy_requirement as b,users as u where u.id=b.creator_id AND ".$like1."";
				
			}else if($keywords=='' && $budgets!='' && $countrys=='' && $categorys=='' && $bsntype==''){
			
				$conditions = "b.budget = '$budgets'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,b.requirements,b.end_date,b.created,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state,u.user_name FROM buy_requirement as b,users as u where  u.id=b.creator_id AND ".$conditions."";
			
			}else if($keywords=='' && $budgets=='' && $countrys!='' && $categorys=='' && $bsntype==''){
				
				$conditions = "u.country_symbol = '$countrys' AND b.creator_id = u.id";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,u.credit,b.created,b.end_date,b.requirements,u.country_symbol,u.user_verify,u.city,u.state  FROM buy_requirement as b,users as u where  u.id=b.creator_id AND ".$conditions."";
			}else if($keywords=='' && $budgets=='' && $countrys=='' && $categorys!='' && $bsntype==''){
			
				$conditions = "b.category = '$categorys'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.created,b.end_date,b.requirements,b.requirement_image,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state,u.user_name  FROM buy_requirement as b ,users as u where u.id=b.creator_id AND  ".$conditions."";
			
			}else if($keywords=='' && $budgets=='' && $countrys=='' && $categorys=='' && $bsntype!=''){
				 
				 $conditions = "b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.created,b.end_date,b.requirements,b.requirement_image,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state,u.user_name  FROM buy_requirement as b ,users as u where u.id=b.creator_id AND  ".$conditions."";
			}else if($keywords!='' &&  $budgets!='' && $countrys!='' && $categorys!='' && $bsntype!=''){
			
			
					 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' AND b.budget = '$budgets')  AND u.country_symbol = '$countrys' AND b.creator_id = u.id AND b.category = '$categorys' AND b.requirements = '$bsntype'";
						 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
						 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			}else if($keywords!='' &&  $budgets!='' && $countrys=='' && $categorys==''  && $bsntype==''){
					
					$conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' AND b.budget = '$budgets') AND b.creator_id = u.id ";
					 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
					 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			}else if($keywords!='' &&  $budgets=='' && $countrys!='' && $categorys=='' && $bsntype==''){
			
				$conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%')  AND u.country_symbol = '$countrys' ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords!='' &&  $budgets=='' && $countrys=='' && $categorys!='' && $bsntype==''){
			
			 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' )   AND b.category = '$categorys'  AND b.creator_id = u.id";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			 ...
			 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' )    AND b.creator_id = u.id AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				...
				$conditions ="(b.budget = '$budgets')  AND u.country_symbol = '$countrys' AND b.creator_id = u.id ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				
			}else if($keywords=='' &&  $budgets!='' && $countrys=='' && $categorys!='' && $bsntype==''){
				
				$conditions ="(b.budget = '$budgets')  AND b.creator_id = u.id AND b.category = '$categorys'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				...
				$conditions ="(b.budget = '$budgets')  AND b.creator_id = u.id   AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				...
				$conditions =" u.country_symbol = '$countrys' AND b.creator_id = u.id AND b.category = '$categorys'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords=='' &&  $budgets=='' && $countrys!='' && $categorys=='' && $bsntype!=''){
			
					$conditions =" u.country_symbol = '$countrys' AND b.creator_id = u.id  AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords=='' &&  $budgets=='' && $countrys=='' && $categorys!='' && $bsntype!=''){
			
				$conditions ="  b.creator_id = u.id AND b.category = '$categorys' AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			}else if($keywords=='' &&  $budgets=='' && $countrys!='' && $categorys!='' && $bsntype!=''){
			
				$conditions ="u.country_symbol = '$countrys' AND   b.creator_id = u.id AND b.category = '$categorys' AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords!='' &&  $budgets!='' && $countrys!='' && $categorys=='' && $bsntype==''){
			
				$conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' AND b.budget = '$budgets' )AND u.country_symbol = '$countrys'    AND b.creator_id = u.id ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords!='' &&  $budgets!='' && $countrys!='' && $categorys!='' && $bsntype==''){
			
			 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%' AND b.budget = '$budgets' )AND u.country_symbol = '$countrys'    AND b.creator_id = u.id AND b.category = '$categorys' ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords!='' &&  $budgets=='' && $countrys!='' && $categorys!='' && $bsntype==''){
			
				$conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%'  )    AND b.creator_id = u.id AND u.country_symbol = '$countrys'    AND b.category = '$categorys' AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			
			}else if($keywords=='' &&  $budgets!='' && $countrys!='' && $categorys=='' && $bsntype!=''){
			
				$conditions ="b.budget = '$budgets'  AND u.country_symbol = '$countrys'  AND b.creator_id = u.id  AND b.requirements = '$bsntype'";
				$query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				...
				$conditions ="b.budget = '$budgets'   AND b.creator_id = u.id  AND b.requirements = '$bsntype' AND b.category = '$categorys' ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				...
				$conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%'  ) AND b.budget = '$budgets'   AND b.creator_id = u.id  AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			}else if($keywords!='' &&  $budgets=='' && $countrys=='' && $categorys!='' && $bsntype!='')
			{
			 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%'  )  AND b.creator_id = u.id  AND b.requirements = '$bsntype' AND b.category = '$categorys' AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				 }
				  else if($keywords!='' &&  $budgets=='' && $countrys!='' && $categorys=='' && $bsntype!='')
			{
			 $conditions ="(b.looking_for LIKE '%$keywords%'  or b.category LIKE '%$keywords%' or b.description LIKE '%$keywords%' or b.budget LIKE '%$keywords%'  )  AND b.creator_id = u.id  AND b.requirements = '$bsntype' AND u.country_symbol = '$countrys' AND b.requirements = '$bsntype'";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
				 }
				 else if($keywords=='' &&  $budgets!='' && $countrys!='' && $categorys!='' && $bsntype!='')
			{
			 $conditions =" b.creator_id = u.id  AND b.requirements = '$bsntype' AND u.country_symbol = '$countrys'  AND b.category = '$categorys' ";
				 $query = "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify,
				 u.city,u.state  FROM buy_requirement as b,users as u where ".$conditions."";
			...
			$config['cur_page']     = $start_inbox;
			$this->pagination->initialize($config);		
			$this->outputData['pagination']   = $this->pagination->create_links2(false,'seller/index');
			$this->load->view('view_browse',$this->outputData);
	}
	
	
	function sellerscrollPagination(){
		...
		$query - "SELECT b.id as buy_id,b.creator_id,b.looking_for,b.category,b.description,b.budget,b.requirement_image,u.user_name,b.created,b.requirements,b.end_date,u.credit,u.country_symbol,u.user_verify, u.city,u.state FROM buy_requirement as b,users as u where (b.budget = '<10,000') AND b.creator_id = u.id AND b.requirements =1";
			...
			$result = $res->result();
			$this->outputData['searches']=$this->db->query($query);
			$this->load->view('view_browsepp',$this->outputData);
...
function signup()
	...
	public function search_function_in_controller()
    {
            ...
            $this->load->model('Search_model');
             $this->outputData['search_result'] = $this->search_model->search_user($keyword);
           $this->load->view('view_browse',$this->outputData);
    }
	// --------------------------------------------------------------------
	/**
