 ...
 * getBidsInfo
 *
 * Will return the average bid value a the job
 *
 * @access	public
 * @param	string
 * @return	string
 */
	...
	{
		$CI 	=& get_instance();
		$currency = $CI->db->get_where('settings', array('code' => 'CURRENCY_TYPE'))->row()->string_value;
		$mod 	= $CI->load->model('skills_model');
		$conditions = array('bids.job_id'=>$projectId);
	...
	function getLowestBid($projectId=NULL,$providerId=NULL)
	{
		$CI 	=& get_instance();
		$mod 	= $CI->load->model('skills_model');
		$conditions = array('bids.job_id'=>$projectId,'bids.user_id' => $providerId);
	...
	function getCategoryLinks($categories=NULL)
	{
		...
		foreach($cat as $cate){	
			...
			$comma = "";
			$links .= "<a href='".site_url('job/category/'.urlencode($cate))."'>".$cate."</a>".$comma;
			$i++;
		}
		return $links;
	} //End of getLowestBid function
	...
	function getCurrentStatus($status=NULL,$programmer=NULL,$project_id=NULL)

                {
...

                                {
                                                ...
                                                $stat['message'] = '('.$CI->lang->line('Choosen Employee').':'.$result->user_name.' , '.$CI->lang->line('Waiting For Accept').')';
...

                                {
                                                ...
                                                $condition2 = array('reviews.employee_id' => $result->id,'reviews.review_type' => '2','reviews.hold' => '0','reviews.job_id'=>$project_id);
                                                ...
                                                $conditions = array('transactions.job_id'=>$project_id,'transactions.type'=>'Transfer','transactions.status'=>'Completed');

                                                $payement_status = $CI->account_model->getTransactions($conditions);

                                                if($payement_status->num_rows()>0)

                                                { $stat['message'] = '('.$CI->lang->line('Choosen Employee').':'.$result->user_name.' , '.$CI->lang->line('paid').$rating.')'; }

                                                else

                                                                $stat['message'] = '('.$CI->lang->line('Choosen Employee').':'.$result->user_name.' , '.$CI->lang->line('unpaid').')';
...
/* End of file bids_helper.php */
/* Location: ./application/helpers/bids_helper.php */
?>