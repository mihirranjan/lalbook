<?php  if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	 ...
	 * Get owners's review
	 *
	 * Returns number of bids posted on a job
	 *
	 * @access	public
	 * @param	string
	 * @return	string
   ...
   function getReviewStatus($projectid,$programmer_id)
   {
   		$CI 	=& get_instance();
		$mod 	= $CI->load->model('skills_model');
		$condition2 = array('reviews.job_id' => $projectid,'reviews.employee_id' => $programmer_id,'reviews.review_type' => '2');
   ...
   function getReviewStatusProgrammer($projectid,$programmer_id)
   {
   		$CI 	=& get_instance();
		$mod 	= $CI->load->model('skills_model');
		$condition2 = array('reviews.job_id' => $projectid,'reviews.employee_id' => $programmer_id,'reviews.review_type' => '1');
		$reviewDetails = $CI->skills_model->getReviews($condition2);
